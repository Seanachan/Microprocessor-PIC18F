...
void button_pressed()
{
    ...
    if (mode == 1)
    {
        signed char target_deg = 0;
        if (dir > 0)
        {
            target_deg = get_servo_angle() + step_deg;
            if (target_deg >= 90)
            {
                target_deg = 90;
                dir = -1;
...
void main()
{
    ...
    while (1)
    {
        if (mode == 2)
        {
            // n steps -90 -> 90, but autumatically without button pressed
            signed char target_deg = 0;
            for (int i = 0; i < n; i++)
            {
                if (dir > 0)
