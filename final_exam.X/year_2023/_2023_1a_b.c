...
// blink_half_ticks = 4 → full period 1.0 s
// blink_half_ticks = 2 → 0.5 s
// blink_half_ticks = 1 → 0.25 s
unsigned char blink_half_ticks = 4;
...
void __interrupt(high_priority) H_ISR()
{
    ...
    { // Handle Timer0 overflow interrupt
        ...
        t0_tick++;
        if (t0_tick >= blink_half_ticks)
        {
            t0_tick = 0;
            // Toggle the single LED on RA1
            timer0_time_out();
        }
...
{ // value: 0 ~ 1023
  // Do sth when the variable register changes
     ...
     */
    unsigned char new_state_num = (value * 3) / VR_MAX; // 0~2

    if (new_state_num == 0)
        blink_half_ticks = 4; // 1.0 s
    else if (new_state_num == 1)
        blink_half_ticks = 2; // 0.5 s
    else
        blink_half_ticks = 1; // 0.25 s

    if (state_num != new_state_num)
    {
...
void timer0_time_out()
{
    set_LED(get_LED() ^ 0b001); // toggle LED1
    state_count++;
