...
void main()
{
    ...
    /*
        Design a device that utilize a button and a motor. This device operates in 2 modes.
        Mode 1: When powered on, the motor moves to -45 degrees. Each time the button is pressed,
                the motor toggles between -45 degrees and +45 degrees.
        Mode 2: When powered on, the motor moves to -90 degrees. Each time the button is pressed,
                the motor toggles between -90 degrees and +90 degrees.
        The device starts in Mode 1. Each time the button is pressed, it switches between Mode 1 and Mode 2.
    */
    ...
    while (1)
    {
        int local_deg = deg; // snapshot to avoid reading while ISR changes it
        int rc;
        // printf("Mode: %d, Deg: %d, Dir: %d\n", mode, local_deg, dir);
        if (dir > 0)
        {
            // move to +deg
            rc = set_servo_angle(local_deg);
            if (rc == -1)
                continue; // interrupted by button, new mode will be handled next loop
            if (get_servo_angle() == local_deg)
                dir = -1;
        }
        else
        {
        ...
        // short pause at ends; also respect button interrupt
        if (delay(0.2) == -1)
        {
            continue; // button pressed during delay; loop will see new mode/deg
        }
