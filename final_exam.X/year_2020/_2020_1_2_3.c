  ...
  if (mode == 2)
  {
    ...
    if (new_step - abs(brighness_step) > 5)
    {
      if (brighness_step < 0)
        ...
        brighness_step = new_step;
      printf("new_step: %d, brighness_step: %d\n", new_step, brighness_step);
    }
  }
}
...
{ // get line from keyboard: this function will be called after you click enter
  ...
  else if (strcmp(str, "mode2") == 0)
  {
    ...
    mode = 2;
    printf("Selected: mode %d\n", mode);
    t1_blink_half_ticks = 1;
    timer1_on_off(1);
  }
  else if (strcmp(str, "mode3") == 0)
  {
    timer1_on_off(0);
    mode = 3;
    printf("Selected: mode %d\n", mode);
    t0_blink_half_ticks = 4; // 1s
  }
}
void timer0_time_out()
{
  ...
  {
    printf("Time left: %d\n", time_left);
    set_servo_angle(get_servo_angle() + (180 / (count_from + 1)));
    if (time_left >= 0)
    {
      time_left--;
      if (time_left < 0)
...
void timer1_time_out()
{
  ...
  if (brightness >= VR_MAX)
  {
    brightness = VR_MAX;
    brighness_step = -brighness_step;
  }
