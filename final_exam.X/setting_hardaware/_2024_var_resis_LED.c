...
const unsigned char led_patterns[7] = {7, 3, 1, 0, 4, 6, 7};
//               state:              0  1  2  3  4  5  6
volatile unsigned char vr_bin = 0;            // 0..6 bin from variable resistor
volatile unsigned char offset = 0;            // extra offset from X
volatile unsigned char state_idx = 0;         // (vr_bin + offset) % 7
volatile unsigned char last_state_idx = 0xFF; // for "state changed" detection
void set_LED(int value);

void show_state(unsigned char s)
{
    unsigned char pat = led_patterns[s];

    // Update the 3 LEDs (RA1–RA3)
    set_LED(pat); // template helper: bits → LEDs
    ...
    if (s != last_state_idx)
    {
        ...
        char str[4];
        str[0] = (pat & 0b100) ? 'L' : 'D'; // LED3 (bit2)
        str[1] = (pat & 0b010) ? 'L' : 'D'; // LED2 (bit1)
        str[2] = (pat & 0b001) ? 'L' : 'D'; // LED1 (bit0)
        str[3] = '\0';
...
{ // value: 0 ~ 1023
  // Do sth when the variable register changes
     ...
     */
    // 0..1023 → 0..6
    unsigned char new_vr_bin =
        (unsigned char)((long)value * 7 / (VR_MAX + 1)); // VR_MAX = 1023

    if (new_vr_bin != vr_bin)
    {
        vr_bin = new_vr_bin;

        // Current state = (vr_bin + offset) % 7
        unsigned char s = (vr_bin + offset) % 7;
        state_idx = s;
        show_state(s); // update LEDs + print L/D if changed
    }
}
...
{ // get line from keyboard: this function will be called after you click enter
     ...
     */
    // Parse integer X
    long X = atol(str); // accepts up to 1e9 easily on 32-bit

    if (X <= 0)
    {
        // spec says 0 < X < 1,000,000,000; ignore invalid
        return;
    }

    X = X % 7; // reduce modulo 7 first
    offset = (offset + (unsigned char)X) % 7;

    // Old state was S = (vr_bin + offset_old) % 7.
    // New state is S' = (S + X) % 7 automatically:
    unsigned char s = (vr_bin + offset) % 7;
    state_idx = s;
    show_state(s); // update LEDs + print L/D
}
