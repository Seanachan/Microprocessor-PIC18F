...
{ // value: 0 ~ 1023
  // Do sth when the variable register changes
    ...
    else if (-45 <= VR_value_to_servo_angle(value) && VR_value_to_servo_angle(value) < 0)
    {
        set_LED_separately(0, 0, 0, 1, 1);
    }
    else if (0 <= VR_value_to_servo_angle(value) && VR_value_to_servo_angle(value) < 45)
    {
        set_LED_separately(0, 0, 1, 1, 1);
    }
    else if (45 <= VR_value_to_servo_angle(value) && VR_value_to_servo_angle(value) < 90)
    {
